...
func DecryptEnglishSingleByteXORhex(hex string) (byte, DecryptAttempt, error) {
	bytes, err := HexToBytes(hex)
	if err != nil {
		return 0, DecryptAttempt{}, err
	}
	return DecryptEnglishSingleByteXOR(bytes)
}

func DecryptEnglishSingleByteXOR(bytes []byte) (byte, DecryptAttempt, error) {
	attempts := SbxorTryAsciiKey(bytes)

	bestk := byte(0)
	for k, v := range attempts {
		if v.engScore > attempts[bestk].engScore {
			bestk = k
		}
	}

	return bestk, attempts[bestk], nil
}

func SbxorTryAsciiKey(bytes []byte) map[byte]DecryptAttempt {
	res := make(map[byte]DecryptAttempt, 256)
	for b := 0; b < 256; b++ {
		bb := byte(b)
		decrypted := string(SingleByteXOR(bytes, bb))
		res[bb] = DecryptAttempt{
			decrypted: decrypted,
